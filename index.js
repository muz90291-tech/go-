...
// ---- Tiny HTTP server for Railway's health check ----
// Railway (as a "Web Service") expects something listening on $PORT.
// Without this, Railway may think the deploy is unhealthy and keep
// restarting the container, which makes the bot flicker online/offline.
const PORT = process.env.PORT || 3000;
http
  .createServer((req, res) => {
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end('Nostang bot is running.');
  })
  .listen(PORT, () => console.log(`🌐 Health check server চালু হয়েছে port ${PORT} এ।`));
